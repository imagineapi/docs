...
Without getting into too many details, ImagineAPI has 3 separate services and a database to store data:
...
```
STANDARD_USER_EMAIL=email you will use to log into ImagineAPI user interface
...
- This is the URL you will have to use for ImagineAPI in the `GET`, `POST` requests. By default, the port is `8055` however you can forward traffic to this port and set `PUBLIC_URL` to that:
```
PUBLIC_URL=ImagineAPI url or the URL to your server. Example: http://mj.yellow-lemon.ts.net:8055
```
